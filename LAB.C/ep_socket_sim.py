...
ep_socket_sim.py
────────────────────────────────────────────────────────────────────────
Lab B (EnergyPlus) — Python API 기반 실시간 소켓 시뮬레이터
...
class EPSocketSim:
    ...
    def _recv_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.info(
                    "[Lab B | STEP5 수신] VILLAS → Lab B  seq=%d | 외기=%.2f°C  일사=%.1fW/m²  습도=%.1f%%  설정온도=%.2f°C",
                    seq, vals[0], vals[1],
    ...
    def _on_end_timestep(self, state):
        ...
        if self._ep_tick % self.log_interval == 0:
            logger.info(
                "[Lab B | STEP5 계산] EnergyPlus 연산 → 실내온도=%.2f°C  HVAC전력=%.1fW  (설정온도=%.2f°C)",
                indoor_temp, hvac_power, cool_sp,
            )
            logger.info(
                "[Lab B | STEP6 전송] Lab B → VILLAS(%s:%d)  2ch 전송",
    ...
    def run(self):
        logger.info("=" * 60)
        logger.info(" Lab B — EnergyPlus Socket Sim (STEP5 수신/계산, STEP6 송신)")
...
def main():
    p = argparse.ArgumentParser(
        description="Lab B — EnergyPlus + VILLASnode 소켓 시뮬레이터 (이벤트 구동, STEP5/6)"
    )
